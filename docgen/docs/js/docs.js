...
docsApp.controller.DocsController = function($scope, $location, $window, $http, sections) {
  ...
  $scope.oFiles = [
    ...
    ,{name:'AppStd.css       ',locn:'/apps/tri',type:'msgs  ',desc:'Standard app css'                                             }
    ,{name:'Chqs.htm         ',locn:'/apps'    ,type:'chqs  ',desc:'Cheque writer app page   '            ,explain:'chqs-app'     }
    ,{name:'Chqs.js          ',locn:'/apps'    ,type:'chqs  ',desc:'Cheque writer app support'            ,explain:'chqs-app'     }
    ,{name:'Deps.css         ',locn:'/apps'    ,type:'deps  ',desc:'Deposit app css'                                              }
    ,{name:'Deps.htm         ',locn:'/apps'    ,type:'deps  ',desc:'Deposit app page   '                  ,explain:'deps-app'     }
    ,{name:'Deps.js          ',locn:'/apps'    ,type:'deps  ',desc:'Deposit app support'                  ,explain:'deps-app'     }
    ,{name:'FormCheque.htm   ',locn:'/apps'    ,type:'chqs  ',desc:'Cheque writer app print page   '      ,explain:'chqs-app'     }
    ,{name:'FormCheque.js    ',locn:'/apps'    ,type:'chqs  ',desc:'Cheque writer app print support'      ,explain:'chqs-app'     }
    ,{name:'FormDeposit.htm  ',locn:'/apps'    ,type:'deps  ',desc:'Deposit app print page   '            ,explain:'chqs-app'     }
    ,{name:'FormDeposit.js   ',locn:'/apps'    ,type:'deps  ',desc:'Deposit app print support'            ,explain:'deps-app'     }
    ,{name:'FormDepSum.htm   ',locn:'/apps'    ,type:'deps  ',desc:'Donor Summary print page   '          ,explain:'deps-app'     }
    ,{name:'FormDepSum.js    ',locn:'/apps'    ,type:'deps  ',desc:'Donor Summary print support'          ,explain:'deps-app'     }
    ,{name:'FormEnvelope.htm ',locn:'/apps'    ,type:'deps  ',desc:'Envelope print page   '               ,explain:'deps-app'     }
    ,{name:'FormEnvelope.js  ',locn:'/apps'    ,type:'deps  ',desc:'Envelope print support'               ,explain:'deps-app'     }
    ,{name:'FormHello.htm    ',locn:'/apps'    ,type:'msgs  ',desc:'Hello app print page   '              ,explain:'hello-app'    }
    ,{name:'FormHello.js     ',locn:'/apps'    ,type:'msgs  ',desc:'Hello app print support'              ,explain:'hello-app'    }
    ,{name:'Hello.htm        ',locn:'/apps'    ,type:'msgs  ',desc:'Hello app page'                       ,explain:'hello-app'    }
    ,{name:'Hello.js         ',locn:'/apps'    ,type:'msgs  ',desc:'Hello app support'                    ,explain:'hello-app'    }
    ,{name:'HelloUI.htm      ',locn:'/apps'    ,type:'msgs  ',desc:'HelloUI app page'                     ,explain:'helloui-app'  }
    ,{name:'HelloUI.js       ',locn:'/apps'    ,type:'msgs  ',desc:'HelloUI app support'                  ,explain:'helloui-app'  }
    ,{name:'HelloUI.css      ',locn:'/apps'    ,type:'msgs  ',desc:'HelloUI app css'                      ,explain:'helloui-app'  }
    ...
    ,{name:'TableDonors.js   ',locn:'/apps'    ,type:'deps  ',desc:'Donor Table Definition'               ,explain:'deps-app'     }
    ,{name:'TableEnvelopes.js',locn:'/apps'    ,type:'deps  ',desc:'Envelope Table Definition'            ,explain:'deps-app'     }
    ...
    ,{name:'TRI.js           ',locn:'/apps/tri',type:'sys   ',desc:'API implementation (slow to load)'                            }
  ...
  $scope.showWhen = function(oFile) {
    ...
    if ((oFile.type.indexOf('all') >= 0) && ($scope.oCat.choose.charAt(0) != '.')) return true;
  ...
  $scope.srcLoader = function(event) {
    ...
    if (event.ctrlKey) {
      var sSrc = ""+sLink;
      var nIX = sSrc.indexOf("/apps");
      sSrc = sSrc.substring(nIX);
      window.open("/SrcDisplay.html?src="+sSrc,"_blank");
    } else {
    ...
    function fOK(data,status) {
      ...
      if (oPre) {
        ...
        angular.element(oPre).removeClass('prettyprinted');
        PR.prettyPrint(); // found on stackoverflow
      } else {
  ...
  $scope.closeSections = function() {
    var oS = document.getElementById("close-sections");
    if (oS) {
      var oSpan = angular.element(oS);
      if (oSpan.hasClass("did-hide")) return '';
      log("Closing sections");
      oSpan.addClass("did-hide");
      var oNext = oSpan.parent();
      var sURL = ""+window.location;
      var sKeep = "hideOverview";
      var nIX = sURL.indexOf("#");
      if (nIX > 0) sKeep = "hide"+sURL.substring(nIX+1);
      while(true) {
        //console.log("next %o %s",oNext,sText);
        if (oNext.length == 0) break;
        if (oNext[0].tagName == 'H3') {
          var oA = oNext.find("A");
          if (oA) {
            //log("found %s",oNext.text()+" "+sURL);
            var sText = oNext.text();
            if (sText.substring(0,sKeep.length) != sKeep) {
              $scope.toggleHide({srcElement:oA[0]});
            }
          }
        }
        oNext = oNext.next();
      }
      if (nIX > 0) window.location.hash = "#close-sections";
    }
    return '';
  }
  ...
  $scope.toggleHide = function(event) {
    ...
    var oBase = oA.parent();
    console.log("firefox base=%o oA=%o",{base:oBase},{oA:oA});
  ...
  if (!$location.path() || INDEX_PATH.test($location.path())) {
    ...
    if (oStart.page) {
      // This allows us to override the start page.  Use index.html?page=demo
      // Used for start-up and seo
  ...
  $scope.isFireFox = function() {
    var oNav = window.navigator;
    //console.log("AGENT:"+oNav.userAgent+" NAME:"+oNav.appName+" VERSION:"+oNav.appVersion);
    if (oNav.userAgent.indexOf("Firefox") > 0) return true;
    return false;
  }
